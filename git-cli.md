...
8. Выполни `git status` и убедись что `commit.md` находится в секции `Untracked files` (и подсвечен красным цветом)

9. Добавь `commit.md` в Commit Index: `git add commit.md`

10. Выполни `git status` и убедись что `commit.md` находится в секции `Changes to be commited` (и подсвечен зеленым цветом)
...
12. Выполни `git status`. Убедись, что `commit.md` находится как в Working Directory (секция `Changes not staged for commit`), так и в Commit Index (секция `Changes to be commited`)

13. Открой в VS Code в боковом меню пункт Source Control (`Ctrl + Shift + G`) и убедись, что `commit.md` находится как в верхней части окна, так и в нижней,
...
14. Выполни коммит с сообщением `Add commit.md header` (можешь посмотреть пункт 9, чтобы вспомнить как это делать)

15. В истории коммитов (Git Graph) найди только что созданный коммит и убедись, что в него попали только изменения из Commit Index
