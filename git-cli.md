...
11. Добавь все измененные файлы, т.е. `.gitignore` и `commit.md`, в Commit Index: `git add .`

12. Выполни `git status` и убедись что `.gitignore` и `commit.md` находятся в секции `Changes to be commited`
...
15. Открой в VS Code в боковом меню пункт Source Control (`Ctrl + Shift + G`) и убедись, что `commit.md` находится как в верхнем списке Staged Changes, так и в нижнем списке Changes,
...
16. Выполни коммит с сообщением `Add commit.md header` (можешь посмотреть пункт 5, чтобы вспомнить как это делать)
