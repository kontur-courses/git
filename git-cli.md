...
3. Открой терминал (Git Bash для Windows). Выполни команду  
...
5. Убедись, что появилась скрытая папка `.git`
   ...
   *Для Mac* - чтобы отображались скрытые файлы и папки, нужно в Finder нажать `Cmd + Shift + .`
   ...
   Также можно просмотреть список файлов и папок в текущей директории из консоли с помощью команды  
   `ls -la`
...
3. Открой Git Graph (кнопка в нижней панели VS Code). Убедись, что в нем есть два коммита: `Initial commit` и `README.md`
...
11. Добавь `commit.md` в Commit Index:
...
14. Открой в VS Code вкладку Source Control (`Ctrl + Shift + G`) и убедись, что `commit.md` находится как в верхней части окна, так и в нижней,
...
17. Закоммить изменения с сообщением `Change commit.md`


## Задание 3. Tag (optional)

1. Создай тег с именем `v0.1` с помощью команды `git tag "v0.1"`
2. Теги также можно создавать в Git Graph с помощью контекстного меню


## Задание 4. Feature Branches

1. Создай новую ветку `branch-feature` и перейди на нее:
...
1. Начни вливать `branch-feature` в `master`, как и в прошлый раз.
    Конфликтов в этот раз не будет, поэтому шаг разрешения конфликтов будет пропущен. Таким образом слияние будет выполнено.
...
3. Теперь пришло время добавить в историю эти изменения.
    ...
    Но можно воспользоваться `Ammend Commit`. Эта команда позволяет дополнить последний коммит дополнительными изменениями: `git commit -a`
...
9. Установи тег `old-rebase-feature` на коммит, на который ссылается `rebase-feature`:
   ...
   *В этой команде первый аргумент - имя тега, а второй - идентификатор коммита. В качестве идентификатора коммита можно передать его хеш или ветку, которая на него указывает*
...
3. В результате ты увидишь список коммитов, по которым передвигался `HEAD`.
    Найди в списке действие `commit: Add reflog.md` и скопируй хеш коммита.
...
1. Выполни checkout на ветку `ext/sheet-feature`: 
...
2. Выполни интерактивный rebase `sheet-feature` на `master`:
...
4. В первой строчке файла замени `pick` на `reword`, а последующих строчках замени `pick` на `fixup`.
    Сохрани изменения и закрой открывшийся файл (`Ctrl + F4` на Windows/Linux, `Cmd + W` на Mac). Редактор откроется снова и в нем надо будет ввести новое сообщение для `Sheet markup`. Введи в качестве сообщения `Extension` и закрой файл.
...
7. Удали ветку `sheet-feature`:

  `git branch -D sheet-feature`
...
4. Начни делать push этой ветки.
    `git push` не сработает потому что ветки в удаленном репозитории еще нет. Чтобы она появилась, нужно выполнить команду `git push -u origin upstream-feature`
...
9. Верни изменения из stash. Для этого выполни команду `git stash pop`
...
2. В Git Graph найди коммит `Change reset.md`, выбери его и с помощью контекстного меню выполни `Copy commit hash to clipboard`. Хеш коммита будет скопирован в буфер обмена. После этого выполни

  `git reset --hard COPIED_COMMIT_HASH`
...
1. Найди в Git Graph начальный коммит `Initial commit` и скопируй его хеш. Выполни

  `git reset --soft COPIED_COMMIT_HASH`
