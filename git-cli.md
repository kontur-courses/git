...
4. Открой Git Graph (кнопка в нижней панели VS Code). Убедись, что в нем есть коммит с названием `root`.
...
Если ты открыл этот MD-файл без применения форматирования и видишь бэктики (вот такой символ: `` ` ``),
то копировать надо то, что между ними, но не сами «бэктики»

6. Выполни команду `git status`. В секции `Untracked files` должен появиться `init.md`.  
**Консольные команды здесь и далее удобнее выполнять через встроенный в VS Code терминал!**

7. Выполни команду `git add init.md`
Теперь `init.md` находится в Commit Index

8. Сделай коммит с сообщением `Add init.md`.
Для этого выполни команду  `git commit -m "Add init.md"`
...
11. Выполни `git status` и убедись что `commit.md` находится в секции `Untracked files`

12. Добавь `commit.md` в Commit Index: `git add commit.md`

13. Выполни `git status` и убедись что `commit.md` находится в секции `Changes to be commited`
...
9. Закоммить изменения с сообщением `Replace with bullets`
...
12. Закоммить изменения с сообщением `New commands for commit.md`
