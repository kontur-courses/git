...
7. Выполни команду `git add init.md`. Теперь `init.md` должен находится в Commit Index.

8. Выполни `git status` и убедись что `init.md` находится в секции `Changes to be commited` (и подсвечен зеленым цветом)

*Выглядеть должно так:*  
<img src="https://raw.githubusercontent.com/kontur-courses/git/master/images/cli-images/status-tracked.png">

9. Сделай коммит с сообщением `Add init.md`. Для этого выполни команду  `git commit -m "Add init.md"`.

10. Открой Git Graph и убедись, что коммит появился в истории коммитов

11. Создай файл `commit.md` со следующим содержимым:
```
...
12. Выполни `git status` и убедись что `commit.md` находится в секции `Untracked files` (и подсвечен красным цветом)

13. Добавь `commit.md` в Commit Index: `git add commit.md`

14. Выполни `git status` и убедись что `commit.md` находится в секции `Changes to be commited` (и подсвечен зеленым цветом)

15. Замени содержимое `commit.md` на следующее:
```
## S2. Хранятся состояния директории, постепенная сборка коммита
#### Хранятся файлы, разница вычисляется на лету
#### Commit index для сборки коммита
...
16. Выполни `git status`. Убедись, что `commit.md` находится как в Working Directory (секция `Changes not staged for commit`), так и в Commit Index (секция `Changes to be commited`)

17. Открой в VS Code в боковом меню пункт Source Control (`Ctrl + Shift + G`) и убедись, что `commit.md` находится как в верхней части окна, так и в нижней,
...
18. Выполни коммит с сообщением `Add commit.md header` (можешь посмотреть пункт 8, чтобы вспомнить как это делать)

19. В истории коммитов (Git Graph) найди только что созданный коммит и убедись, что в него попали только изменения из Commit Index

20. Закоммить оставшиеся в Working directory изменения с сообщением `Change commit.md`.
...
Чтобы завершить коммит, достаточно закрыть в редакторе открывшийся файл `COMMIT_EDITMSG` (`Ctrl + F4` на Windows, `Ctrl + W` на Linux, `Cmd + W` на Mac).
