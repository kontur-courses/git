...
> [Как вставить текст из буфера обмена в терминал?](https://www.google.com/search?q=%D0%9A%D0%B0%D0%BA+%D0%B2%D1%81%D1%82%D0%B0%D0%B2%D0%B8%D1%82%D1%8C+%D1%82%D0%B5%D0%BA%D1%81%D1%82+%D0%B8%D0%B7+%D0%B1%D1%83%D1%84%D0%B5%D1%80%D0%B0+%D0%BE%D0%B1%D0%BC%D0%B5%D0%BD%D0%B0+%D0%B2+%D1%82%D0%B5%D1%80%D0%BC%D0%B8%D0%BD%D0%B0%D0%BB)
...
Проверь, что все получилось, выполнив в терминале команду `git status`. Она должна вывести какое-то такое сообщение:
```
On branch main
...
А вот такой вывод команды `git status` указывает на ошибку:
```
fatal: not a git repository (or any of the parent directories): .git
```
В случае ошибки перепроверь предыдущие шаги.
...
Проверь, что настройки подключились успешно. Для этого в терминале в папке с репозиторием выполни команду `git st`.

Если команда вывела `## main...origin/main`, то все хорошо.

Если же вывод команды начинается с `git: 'st' is not a git command`, то настройки не подключились корректно, а значит надо перепроверить предыдущий шаг.
...
Рабочий каталог и репозиторий находятся внутри этой скопированной папки, так что в любой момент можно будет вернуться к сохраненному состоянию. Для этого надо либо переключиться на копию папки, либо заменить все содержимое папки `ulearn-git-guide` на содержимое папки с сохраненным состоянием.
...
Так как ты знаешь весь расклад, то можешь предположить, что при слиянии веток `quotation-feature` и `main` произойдет конфликт, потому что в файл `s2.md` вносились правки в обеих ветках. Разрешать конфликт будешь на следующем шаге, а пока просто начни слияние.
...
Влей `merge-feature` в `main` в режиме fast-forward.

> По умолчанию Git использует fast-forward, когда это возможно. Но в графических интерфейсах для Git по умолчанию может быть выставлена опция, запрещающая fast-forward и обязывающая создавать новый коммит при каждом слиянии. Если используешь графический интерфейс для выполнения слияния, убедись, что опции, обязывающие создавать новый коммит, не выбраны.

### Шаг 7

Если все сделать правильно, то слияние пройдет без конфликтов и без создания нового коммита!
...
Убедись, новый коммит действительно не был создан.
...
Когда разработка новой функциональности в ветке происходит довольно долго, бывает полезно получать свежие правки из основной ветки. Это один из сценариев, где пригождается команда rebase.
...
Для этого переключись на ветку `main` и влей в нее ветку `picking-feature`. Слияние должно пройти в режиме fast-forward.
...
`git remote add ext https://github.com/kontur-courses/ulearn-git-guide-ext.git`
...
Переключись на ветку `main` и влей в нее изменения из `guide-feature`. Слияние должно пройти в режиме fast-forward.
